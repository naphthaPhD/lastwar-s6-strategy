...
## 2026-05-30 #534-only invasion risk update

## Context

味方グループ全体の侵攻リスクとは別に、#534/JDX 保有拠点だけを対象にした侵攻リスク表示を追加した。
...
1. Phase3 に `self_risk_watchlist` を追加し、#534/JDX 保有拠点のみの侵攻リスクを抽出できるようにした。
2. UI に `#534侵攻リスク` と `#534協定込みリスク` ボタンを追加した。
3. `briefing_input.json` に `self_risk_watchlist` と `top_self_pact_threats` を追加した。

## Notes

- 2026-05-30 の再生成では `self_risk_watchlist` は30件。
- `self_enemy_threat_options` は30件。
- `self_pact_threat_options` は0件。現時点の協定データでは、協定経由だけで #534/JDX 保有拠点へ新しく届く候補は出ていない。
