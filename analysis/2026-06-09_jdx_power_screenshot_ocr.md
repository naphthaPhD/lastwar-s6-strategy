...
Google Sheets `JDX戦力など` の `現在週` へ直接反映する更新候補を作成した。初回はOCR名寄せに不安定な候補が含まれるため未実行としたが、その後ユーザーの明示指示により、`data/2026-06-09_jdx_power_sheet_update_plan.csv` の変更候補を一括反映した。
...
- OCR抽出結果:
  ...
  - シート名簿に寄せた更新候補: 73名
  - 既存シートとの差分候補: 72名
- Google Sheets `戦力分析` は、OCR反映後に `2026/06/09 21:26:01` に再計算済みだった。
...
- 2026-06-09 19:43以降: 初回のシート一括更新は安全性確認のため未実行。
- 2026-06-09 21:26: ユーザーの明示指示により、OCR更新候補72名・345セルを `現在週!C:G` へ反映。
...
1. 総戦力が大きく減る候補と名前OCRが弱い候補を、画像でスポットチェックする。
2. 反映後の `戦力分析` を使って、Week 5向けのJDX戦力配分を検討する。
3. 反映完了後、今回分 `IMG_8159.PNG` - `IMG_8191.PNG` を `processed` または `処理済み` へ移動する。

## 8. Unknowns

- 総戦力が前回より下がっている候補が、実際の戦力変動かOCR/名寄せミスか。
- コメント欄でOCRが編成を読めなかった候補の正しい編成。
- 大幅な総戦力増減が、実変動かOCR/名寄せミスか。
