...
- `漁場一覧`, `イベント一覧`, `シミュレーター`, `カレンダー`, `ライン定義`, `侵攻ルート確認` の6シートを作成・更新する設計。
...
3. `侵攻ルート確認` で同じ開放枠が連続しているルートを優先確認する。
4. 実画面の保護終了表示と計算結果を数件突き合わせる。
5. 放棄を1件テストできるタイミングで、放棄後保護終了が Apps Script の仮定と一致するか確認する。
