...
- Node colors are strategic colors: #534-side owners are blue, #509/#440/#511-side owners are green, #503/#480/#523/#476-side owners are red, trade posts are black, destroyed cities are gray, and unowned nodes are white. Owners with server-number prefixes such as `476B` are classified by that prefix even when they occupy another area.
...
- `sample_output/briefing_input.json` is a compressed Phase 3 briefing input for future GPT use. It contains only strategic top lists, score reasons, assumptions, missing data, and summary metadata instead of the full map state.
- The Phase 3 rule engine v2 scores nodes with JSON reasons and factor breakdowns for `choke_score`, `isolation_score`, `coalition_score`, `alliance_score`, `invasion_score`, `protection_score`, and `time_score`. It also preserves the previous edge-candidate lists and boundary/interior-depth counts used by the map highlight buttons.
- The generated map includes Phase 3 score buttons: `Phase3防衛TOP`, `Phase3攻撃TOP`, `Phase3遮断TOP`, `Phase3危険TOP`, `Phase3保護TOP`, `Phase3時間TOP`, and `Phase3総合TOP`. These open a score panel from the embedded `invasion_simulation` JSON, list the top candidates with score reasons, and highlight/select the corresponding edge or node.
