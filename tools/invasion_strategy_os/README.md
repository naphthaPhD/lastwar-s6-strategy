...
- `ルート選択` enables shortest-route selection: click one node as the start, click a second node as the target, and the shortest route between them is highlighted in purple with thicker edges. `ルート解除` clears the route.
