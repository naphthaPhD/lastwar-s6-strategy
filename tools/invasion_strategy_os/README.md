...
- Selecting a fishery highlights all edges connected to that fishery in cyan. `境界強調` highlights fishery edges where a friendly-side node (#534/#509/#440/#511 group) touches an enemy-side node in orange, and where an enemy-side node touches an unowned node in amber. `境界+内側` also highlights fishery edges from those friendly/enemy boundary friendly-side fisheries into friendly-side or unowned fisheries in rose red. `強調解除` clears edge highlighting.
- `ルート選択` enables shortest-route selection on fishery routes only: click one node as the start, click a second node as the target, and all equal-length shortest-route fishery edges are highlighted in purple with thicker edges. Cities are not used as transit nodes. If the target is a city, the route is highlighted to the nearest fishery adjacent to that city. Route-mode node selection suppresses the normal yellow adjacent-edge selection highlight. `ルート解除` clears the route.
